...
Zadání 1: Vytvořte stavy pro login a heslo a propojte je obousměrně s příslušnými inputy
  ve formuláři.
Zadání 2: Na formulář pověstě obsluhu události `onSubmit`. Nezapomeňte na `e.preventDefault`.
  Po odeslání formuláře vypište do konzole objekt se dvěma vlastnostmi: `login` a `password`,
  které budou obsahovat aktuální hodnoty z inputů.
...
export const Ukol4 = () => {
  ...
  const kontrolaLogin = (l) => {
    l === ''? setError('Chybi Login') : setError('Login ok')
  }
...
<div className="error" >{kontrolaLogin(l)}</div>
     ...
     // setHeslo(heslo);
      ...
      console.log(heslo)}}>
        ...
        <div className="field">
          <label>
            Heslo:
            <input type="password" onChange={(e) => setHeslo(e.target.value)}/>
          </label>
        </div>
